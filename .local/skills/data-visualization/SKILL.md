...
to Databricks's own platform with zero egress), follow the `databricks-app` skill for the
SQL discipline and caching rules that apply there. Those apps get Replit's built-in PostgreSQL database in
development, and Publish provisions or reuses a production Lakebase database and binds it to
the deployed app, so `api_cache` and other app-owned tables belong there -- use the
`database` skill to work with it.
