...
- `NO_SLIDES_ARTIFACT` -> If the user clearly wants a deck exported but none exists, follow the pre-generation flow in `../SKILL.md` first. Resolve `slideFormat` from the user's choice and availability; do not infer it from PPTX or PDF. Then call `createArtifact({ artifactType: "slides", slideFormat, ... })` and finish building before exporting. If multiple artifacts exist and you picked the wrong one, retry with the correct `artifactDirName` instead of creating another deck.
