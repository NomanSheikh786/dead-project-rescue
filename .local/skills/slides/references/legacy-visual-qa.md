...
- **`overflow: scroll` to "hide" text that overflows the slide frame.** Slides don't scroll. If body text overflows, fix the content. Internal scroll on table/chart containers (per the `<legacy_constraints>` allowed-interactive list in `legacy-building.md`) is fine — the rule is no scroll on the *slide* itself.
