...
Execute these steps in order. No additional steps. Step 0 runs only when the request calls for a real brand's identity; every later step always runs.

Before step 0, `selectedVideoAspectRatio` must hold one exact ratio, resolved per the ratio step in `.local/skills/video-js/SKILL.md`. If the ratio is still missing or ambiguous, ask the required question from `.local/skills/video-js/references/resolution.md` now, end your turn, and continue only after the user answers. No research, downloads, or file changes before the ratio is resolved.

### Step 0: Pull brand media from the web (only when the request needs it)

If the request names a real company, product, or brand whose visual identity should appear (e.g. "use our fonts and logos") and the user did not attach the brand assets, gather them now -- the design subagent cannot call these callbacks itself:
...
Skip this step when no real brand is involved, when the user attached the brand assets, or when the callbacks are unavailable or fail -- go straight to step 1.

### Step 1: Delegate to DESIGN subagent

Call `subagent()` and do not await it. Pass the user's request exactly as-is, prefixed with the skill reference and followed by the authoritative ratio instruction. If step 0 gathered brand assets, pass the downloaded file paths in `relevantFiles` and append a `Brand assets` block to the task -- distilled facts only (file labels, font names, hex codes, source URL). Never paste raw `extractBranding` or search output into the task, and never pass an image URL instead of a downloaded file: an image that is not a workspace file is unavailable. Do NOT add creative direction, color guidance, typography choices, or any other prescriptive instructions beyond that block.

```javascript
const videoDesignTask = `You are the subagent executing this task, see the skill: .local/skills/video-js/SKILL.md\n\n[Insert user request...]\n\nBrand assets (downloaded to the workspace):\n- attached_assets/<company>-logo.svg -- primary logo, from <official source url>\n- Brand fonts: <names, closest Google Fonts match if proprietary>\n- Brand colors: <hex codes>\n\nUse the ${selectedVideoAspectRatio} aspect ratio. This ratio is authoritative. Persist it before writing the composition.`;
subagent({
    name: "video-design",
    task: videoDesignTask,
    config: {
        $kind: "design",
        relevantFiles: [".local/skills/video-js/SKILL.md", ".local/skills/video-js/references/resolution.md", ".local/skills/video-js/references/finalize-playback.md", "attached_assets/<company>-logo.svg"],
    },
});
```

Omit the `Brand assets` block (and the extra `relevantFiles` entry) when step 0 was skipped.
...
- Do NOT read any files before delegation to the design subagent (except `.local/skills/video-js/references/resolution.md` when the ratio question is still required) -- scaffold files are documented above, and the subagent's output does not need verification.
- Do NOT add your own creative direction, style instructions, color guidance, or typography choices to the subagent task. The design subagent is the creative expert -- let it make all decisions. The only exception is the `Brand assets` block from step 0: downloaded logo files, brand font names, and brand hex codes are facts, not creative direction.
