...
- Add `const [muted, setMuted] = useState(false);` in the iframe branch.
...
Control bar order should be: play/pause button, scene-lock button, audio mute button, divider, segmented progress bar, scene counter, elapsed/total playback timer, collapse chevron.
...
If the artifact's `src/lib/video/hooks.ts` predates the `paused` option (no `paused` field on `UseVideoPlayerOptions`), the play/pause wiring was skipped per `scene-selectors.md` — wire audio without it: omit the `paused` prop and the `paused` branches below, and keep the plain seek-on-scene-change effect. Do not modify `hooks.ts` to add the option.
...
- [ ] Iframe preview defaults to unmuted and exposes a `Volume2` / `VolumeX` control.
...
- [ ] Pausing playback pauses the audio; resuming continues from the frozen timestamp without seeking back.
